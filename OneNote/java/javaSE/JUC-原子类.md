...
前面我们说到,如果要保证 `i++` 的原子性,那么我们的唯一选择就是加锁,那么,除了加锁(详见 [[JUC-锁]])之外,还有没有其他更好的解决方法呢?JUC 为我们提供了原子类,底层采用 CAS 算法,它是一种用法简单、性能高效、线程安全地更新变量的方式。
...
CAS 同样是 [[JUC-AQS]] 与 [[JUC-锁]] 底层实现的核心机制,是 JUC 并发体系的基石。
