...
决策树是一种树形结构。树中每个内部节点表示一个特征上的判断，每个分支代表一个判断结果的输出，每个叶子节点代表一种分类结果。本节是 [[概述]] 中提到的监督学习分类算法之一。
...
决策树也易过拟合，采用剪枝的方法缓解过拟合。多个决策树的集成参见 [[集成学习]] 中的随机森林。
...
1. 计算（信息）熵：`熵 = -1/2 * log2(1/2) * 2 = 1`
2. 计算条件熵：
   - `α 的条件熵 = -1/4 * log2(1/4) + -3/4 * log2(3/4) = 0.81`
   - `β 的条件熵 = -2/2 * log2(2/2) = 0`
   - `条件熵 = α 占比 * α 的条件熵 + β 的占比 * β 的条件熵 = 4/6 * 0.81 + 2/6 * 0 = 0.54`
3. 计算信息增益：`信息增益 = 熵 - 条件熵 = 1 - 0.54 = 0.46`
...
| | |
| --- | --- |
| ![CART 1](_assets/决策树/决策树__11-49-24-5.png) | **是否有房** 基尼指数计算过程 |
| **婚姻状况** 基尼指数计算过程 | ![CART 4](_assets/决策树/决策树__11-49-28-8.png) |

#### 是否有房

- 有房子的基尼值：有房子有 1、4、7 共计三个样本，对应：3 个 no、0 个 yes

  `Gini(是否有房, yes) = 1 - (0/3)² - (3/3)² = 0`
- 无房子的基尼值：无房子有 2、3、5、6、8、9、10 共七个样本，对应：4 个 no、3 个 yes

  `Gini(是否有房, no) = 1 - (3/7)² - (4/7)² = 0.4898`
- 基尼系数：第一部分样本占了总样本的 3/10、第二部分样本占了总样本的 7/10

  `Gini_index(D, 是否有房) = 7/10 * 0.4898 + 3/10 * 0 = 0.343`
...
1. 计算 (married) 和 (single, divorced) 情况下的基尼指数
   ...
   - 不结婚的基尼值，有 1、3、5、7、8、10 共 6 个样本，并且对应 3 个 no、3 个 yes：
     ...
     `Gini_index(D, single, divorced) = 1 - (3/6)² - (3/6)² = 0.5`
   - 以 married 作为分裂点的基尼指数：

     `Gini_index(D, married) = 4/10 * 0 + 6/10 * [1 - (3/6)² - (3/6)²] = 0.3`
2. 计算 (single) | (married, divorced) 情况下基尼指数

   `Gini_index(D, 婚姻状况) = 4/10 * 0.5 + 6/10 * [1 - (1/6)² - (5/6)²] = 0.367`
3. 计算 (divorced) | (single, married) 情况下基尼指数

   `Gini_index(D, 婚姻状况) = 2/10 * 0.5 + 8/10 * [1 - (2/8)² - (6/8)²] = 0.4`
...
![回归树 1](_assets/决策树/决策树__11-49-30-9.png) ![回归树 2](_assets/决策树/决策树__11-49-31-10.png) ![回归树 4](_assets/决策树/决策树__11-49-38-12.png)

假设在生成 3 个区域之后停止划分，以上就是最终回归树。每一个叶子节点的输出为：挂在该节点上的所有样本均值。
