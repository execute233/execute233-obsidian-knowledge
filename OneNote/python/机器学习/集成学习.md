...
集成学习是机器学习中的一种思想（详见 [[概述]]），它通过多个模型的组合形成一个精度更高的模型，参与组合的模型称为基学习器（弱学习器）。训练时，使用训练集依次训练出这些弱基学习器，对未知的样本进行预测时，使用这些基学习器联合进行预测。
...
- 若三次预测的结果**串联**起来：$80 + 16 + 3.2 = 99.2$

通过**拟合残差**可将多个弱学习器组成一个强学习器，这就是提升树的最朴素思想。
...
**初始化弱学习器（CART 树）**：当模型预测值为何值时，会使得初始弱学习器的平方误差最小，即求损失函数对 $f(x_i)$ 的导数，并令导数为 0。

$$L(y, f(x)) = \frac{1}{2} \sum_{i=1}^{n} (y_i - f(x_i))^2 \quad \text{求平方误差最小}$$

$$\frac{\partial L(y, f(x))}{\partial f(x_i)} = \sum_{i=1}^{n} (y_i - f(x_i)) \cdot (y_i - f(x_i))' = \sum_{i=1}^{n} (y_i - f(x_i)) \cdot (-1) = -\sum_{i=1}^{n} (y_i - f(x_i)) = 0$$

$$nf(x_i) = \sum_{i=1}^{n} y_i \quad \Rightarrow \quad f(x_i) = \frac{\sum_{i=1}^{n} y_i}{n}$$

从公式可得：**初始化树桩输出值为目标值的均值时**，可使初始弱学习器的平方误差最小。

均值 $f(x) = \frac{5.56+5.70+5.91+6.40+6.80+7.05+8.90+8.70+9.00+9.05}{10} = 7.31$，10 个样本的初始预测值均为 7.31。

**构建第 1 个弱学习器**，根据负梯度的计算方法得到下表：
...
当 1.5 为切分点：拟合负梯度 $-1.75, -1.61, -1.40, -0.91, \ldots, 1.74$。

- 左子树：1 个样本 $-1.75$；右子树 9 个样本：$-1.61, -1.40, -0.91, \ldots$
- 右子树均值：$\frac{(-1.61)+(-1.40)+(-0.91)+(-0.51)+(-0.26)+1.59+1.39+1.69+1.74}{9} = 0.19$；左子树均值为 $-1.75$
- 计算平方损失：

$$\begin{aligned}
&(-1.61-0.19)(-1.61-0.19) + (-1.40-0.19)(-1.40-0.19) + (-0.91-0.19)(-0.91-0.19) \\
&+ (-0.51-0.19)(-0.51-0.19) + (-0.26-0.19)(-0.26-0.19) \\
&+ (1.59-0.19)(1.59-0.19) + (1.39-0.19)(1.39-0.19) + (1.69-0.19)(1.69-0.19) \\
&+ (1.74-0.19)(1.74-0.19) = 15.72308
\end{aligned}$$
...
当 6.5 作为切分点时，平方损失最小，此时得到第 1 棵决策树：左子树 6 个样本，输出 $-1.07$；右子树 4 个样本，输出 $1.60$。
...
以 3.5 作为切分点时，平方损失最小，得到第 2 棵决策树：左子树 3 个样本，输出 $-0.52$；右子树 7 个样本，输出 $0.22$。
...
以 6.5 作为切分点时，平方损失最小，得到第 3 棵决策树：左子树 6 个样本，输出 $0.15$；右子树 4 个样本，输出 $-0.22$。
