...
**Series****对象**：类似于数组，由index和value两个数组组成  
访问方式跟字典差不多  
...
|   |   |   |
|---|---|---|
|index 索引|values ndarray数据|head(n) 前n行|
|tail(n)后n行|dtype数据类型|shape形状(行数)|
|describe()统计描述|isnull()元素是否全为 NaN|notnull()|
|unique()去重|value_counts()每个唯一值的出现次数|map(func) forEach|
|apply(func) forEach|astype(dtype)转换为指定的类型|sort_values()按值排序|
|sort_index()索引进行排序|dropna()删除缺失值（NaN）|fillna(value)填充缺失值|
|replace(to_replace, value)  <br>替换指定的值|cumsum()累计求和|cumprod()累计乘积|
|shift(periods)  <br>元素按指定的步数进行位移|rank()排名|corr(other)  <br>皮尔逊相关系数|
|cov(other)协方差|to_list()|to_frame()|
|iloc[]  <br>通过位置索引来选择数据|loc[]通过标签索引来选择数据||
...
字典的key就是字段名，而其value是一列数据，比如：  
data={"语文": [97, 95, 93], "数学": [99, 92, 95], "英语": [99, 95, 97]}其DataFrame就是  
每个元组都是一行数据，字段名会自动生成，比如：  
data = [(97, 99, 99), (95, 92, 95), (93, 95, 97)]等同于上表的数据  
直接将ndarray作为data传入  
如何改名：  
df.colummns = 新的columns或者使用df.rename(index={指定索引: "指定值"}, inplcae=True)用于精准修改，inplace确认是否要替换原来的  
有下面一堆常用的对象方法

|   |   |   |
|---|---|---|
|head(n=5)前n行数据|tail(n=5)后n行数据|info()简要信息|
|describe()统计信息|shape行列数|columns所有列名|
|index所有索引|dtypes每列的数据类型|sort_values(by)按指定列排序|
|sort_index()按索引排序|dropna()删除含NaN的行或列|fillna(value)用指定值填充缺失值|
|isnull()判断缺失值  <br>返回Bool类型的DataFrame|notnull()与左边相反|loc[]按标签索引选择数据|
|iloc[]按位置索引选择数据|at[]访问单个元素|iat[]访问单个元素|
|apply(func)  <br>对每个元素应用一个函数|applymap(func)同左边一样|groupby(by)分组操作|
|pivot_table()创建透视表|merge()合并多个DataFrame  <br>类似与SQL的JOIN|concat()按行或列连接  <br>多个DataFrame|
|to_csv(),to_excel(),to_json()|query()用SQL风格查询|duplicated()  <br>返回Bool的DataFrame  <br>指示每行是否重复|
|drop_duplicates()  <br>删除重复的行|set_index()设置指定的列为索引|reset_index()重置索引|
|transpose()转置（行列交换）|drop()删行0或列1,用axis指定||

**数据类型**  
DataFrame或Series中的类型有很多，包括了：  
Object - 字符串  
int，float，bool，nan  
datetime - 日期时间  
timedelta - 时间差  
...
**索引操作**  
先列后行（先写是哪个列，再是哪个行），如  
df["time"] # 获得名称为time的Series对象  
df["time"]["count"] # 先拿到名称为time的Series对象，然后砸到名为count的列，获取单元格内容  
结合loc与iloc  
df.loc[行索引，列名]  
df.iloc[行号，列索引]  
也可以使用切部分数据操作  
df.loc[行索引开始: 行索引结束, [指定的那些列名]] # 用来挖出DataFrame的一小块区域,行两边都包，同样的，iloc也可以这样做[start: end, start: end]，一样行两边都包，但都包左不包右  
**赋值操作**  
简单，直接=就行  
...
Series排序  
Series也可以.sort_index或者.sort_values来排序  
**DataFrame****运算**  
**Series对象**  
add() 对该Series对象所有数据相加操作，等同于 series + x，需要用变量接收数据  
low() 减法，乘除略  
series 逻辑运算符 指定值，返回一个新的Series对象，里面存储bool指示是否满足条件，塞DF里  
isin([…]) 是否在某个值,相当于series == 值1 | series == 值2 | …  
**DataFrame对象**  
df[df[指定列名] 逻辑运算符 值] 筛选DF里面指定列中满足指定条件的所有行组成新的DF  
复杂多个条件逻辑判断需要()指定优先级  
同样，可以用query函数写，如.query("列名 \> xx & 列名 \< xx"), query("列名 in [xxx]")  
**统计运算**  
df.describe() # 获得很多统计结果，count，mean，std，min，max等  
df.sum() # 针对于每列求和，对Series引用就是对其里面的数据求和，下面几个函数一样  
mean() # 平均值  
median() # 中位数  
cumsun() # 逐个累计求和，返回Series或DataFrame  
**apply自定义运算**  
apply(func, axis=0) # 默认0是列，1是行  
**文件读取与存储**  
**CSV**  
...
我们可以使用drop删除指定的行0或列1，用axis指定，也可以用columns或index指定  
**Excel**  
pd.read_excel(io) 读取excel文件，返回DataFrame，有以下可选参数  
...
.parse(sheet_name) - 解析指定表单并返回DataFrame  
.close()  
...
mode：默认为 'w'，表示写入模式。如果设置为 'a'，则表示追加模式，向现有文件中添加数据（仅支持部分引擎，如 openpyxl）。  
...
keep_default_na=True: 是否保留默认的缺失值标记,如NaN  
line=False: 是否逐行读取json对象，即每行都是独立的json,把[]用行分割了  
常见的 orient 参数选项:  
...
df["列名"] = [] # 直接给列表赋值  
df["列名"] = df["列名"] 运算 值 # 也可通过获得的其他Series进行计算赋值  
df.assign(c1=…, [c2=…, c3=…]) # 直接添加一个或n个列  
df["列名"] = np.where(series == ?, true_result, false_result) # 根据某列的值判断增加新的列  
**删除列**  
df.drop(axis="columns"或"index") # 除非指定inplace，不会修改原数据  
def df["列名"] # 删除列，会修改原数据，要删多个里面再套个列表  
**去重**  
df.drop_duplicates() # DataFrame去重，就是行去重，删去重复的行，可以inplace  
series.drop_duplicates()  
**修改**  
df["已有列名"] = xxx # 修改已有列的数据  
df["列名"].replace(old, new) # 修改指定单元格的数据，注意replace  
**查询**  
df.sort_values([…]) # 指定一行或多行里的数据进行排序  
df[["列名1", …]] # 获取单个或多个行，返回DataFrame  
df.query() # 一样的  
**排序**  
df.rank() # 针对每列都会排名  
可指定method：  
...
dense - 连续，数值相同评分一致  
为解决上面问题，我们可以对单独的series.rank(method)  
**高级处理 - 缺失值处理**  
判断缺失值  
pd.isnull() # 判断是否有缺失值，传入DataFrame或Series，返回bool版本的DataFrame或Series  
删除缺失值  
df.dropna() # 删除缺失值，默认行，注意inplace  
ap.all(pd.notnull(…)) # 判断某列是否是包含缺失值的列，阵列都是True才是True  
填充缺失值  
df.fillna() # 将缺失值全部替换为指定值  
df.["列名"].fillna(df["列名"].mean()) # 替换指定列的NaN为平均值，注意inplace  
也可以使用for series in df.columns循环获取列名来填充每个列的NaN为平均值  
np.all(pd.notnull(df/series)) # 只有全部为True才为True  
非NaN的转化  
比如有?这样的值，需要替换它  
replace('?', np.nan) # 全部替换为nan再进行操作  
**高级处理 - 数据合并**  
pd.concat([data1, data2], axis=) # 按行或列进行合并，默认outer，不能right或left  
pd.merge(left, right, how='innner', on=None) # on为关联哪些字段  
**高级处理 - 数据分组**  
df.group("列名") # 指定按列或多列进行分组,返回DataFrameGroupBy对象  
分组聚合  
DataFrameGroupBy() # 获取指定的分组，有多个行进行分组就用多个  
DataFrameGroupBy.agg({'列名1': '聚合函数名', '列名2': '聚合函数名'}) # 聚合函数  
DataFrameGroupBy["列名"].聚合函数() # 同上，返回Series，[[]]可以返回DaaFrame  
分组过滤  
DataFrameGroupBy.filter(lambda x: x["指定列"].针对列的计算方法 条件表达式)  
等价于 DataFrameGroupBy["列名"].filter(lambda x: x.针对列的计算方法 条件表达式)  
等价于 df.query()  
x是除了分组列的DataFrame对象，这里是筛选出符合条件的行  
**高级处理 - 交叉表与透视表**  
data = {  
"性别": ["男", "女", "男", "女", "男", "女"],  
"购买": ["是", "否", "是", "是", "否", "否"]  
...
透视表：指定某一列对于另外一列的关系  
也可以实现：  
df.pivot_table(index="性别", columns="购买", value="购买", aggf="count")  
aggf可以替换为其他的，mean min都行  
**绘图**  
DataFrame.plot() # 绘制折线图，第一列作为x轴，第二列作为y轴  
DataFrame.plot.bar(color=[]) # 柱状图  
对于相同的参数，可以定义map存储，传参只要**解开就行，如  
...
找到最新的数据  
参照上面，对时间进行比较即可，建议使用sort_values